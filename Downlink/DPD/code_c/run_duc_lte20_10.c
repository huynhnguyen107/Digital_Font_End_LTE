...
#include "lte20_slot_q15_10.h"// for the input data slot 0xF000 bytes
#include "lte20_duc_golden_q15_10.h"// for the golden output data slot
...
int main(void) {
    ...
    if (LTE20_SLOT_SAMPLES !=10) {
        printf("Error: Unexpected number of input samples. Expected 10, got %d\n", LTE20_SLOT_SAMPLES);
        return -1;
    }
    if (LTE20_DUC_OUT_SAMPLES != 40) {
        printf("Error: Unexpected number of output samples. Expected 40, got %d\n", LTE20_DUC_OUT_SAMPLES);
        return -1;
    }
    if(OUTPUT_OFFSET+LTE20_DUC_OUT_BYTES > BRAM_MAP_SIZE) {
        printf("Error: Output data exceeds BRAM map size. Output end address: 0x%X, BRAM map size: 0x%X\n", OUTPUT_OFFSET + LTE20_DUC_OUT_BYTES, BRAM_MAP_SIZE);
        return -1;
    }
    //open /dev/mem to access physical memory
    ...
    DMA_REG(dma, DMA_S2MM_LENGTH_OFFSET) = LTE20_DUC_OUT_BYTES; // Set S2MM transfer length
    ...
    for (size_t i = 0; i < LTE20_DUC_OUT_SAMPLES; i++) {
        ...
        uint32_t golden_sample = lte20_duc_golden_q15[i];

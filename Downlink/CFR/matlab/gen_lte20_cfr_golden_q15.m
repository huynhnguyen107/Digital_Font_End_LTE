...
OUT_TXT = 'lte20_duc_cfr_golden_q15.txt';
OUT_BIN = 'lte20_duc_cfr_golden_q15.bin';
OUT_H   = 'lte20_duc_cfr_golden_q15.h';

REPORT_TXT = 'lte20_duc_cfr_papr_report.txt';
...
write_header(OUT_H, 'lte20_duc_cfr_golden_q15', words_out);
...
papr_cfr_db = calc_papr_db(x_out);
papr_reduction_db = papr_duc_db - papr_cfr_db;

peak_duc = max(abs(x_duc));
peak_cfr = max(abs(x_out));

rms_duc = sqrt(mean(abs(x_duc).^2));
rms_cfr = sqrt(mean(abs(x_out).^2));
...
fprintf('\n');
fprintf('PAPR reduction       : %.2f dB\n', papr_reduction_db);
fprintf('I clip count         : %d\n', i_clip_count);
fprintf('Q clip count         : %d\n', q_clip_count);
fprintf('IQ sample clip count : %d\n', iq_clip_count);
fprintf('=============================================\n');

%% Write PAPR report TXT
fid = fopen(REPORT_TXT, 'w');

fprintf(fid, 'AXI-DMA-Based LTE 20 MHz DUC+CFR PAPR Report\n');
fprintf(fid, '============================================\n\n');
...
fprintf(fid, 'PAPR reduction       : %.2f dB\n', papr_reduction_db);
...
function write_header(filename, array_name, words)
    ...
    fprintf(fid, '#define LTE20_DUC_CFR_OUT_SAMPLES %d\n', length(words));
    fprintf(fid, '#define LTE20_DUC_CFR_OUT_BYTES   (LTE20_DUC_CFR_OUT_SAMPLES * 4)\n\n');

    fprintf(fid, 'static const uint32_t %s[LTE20_DUC_CFR_OUT_SAMPLES] = {\n', array_name);

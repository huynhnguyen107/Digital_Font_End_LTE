...
#include "lte20_ul_if_q15.h"// for the input data slot 245760 bytes
#include "lte20_ul_mixer_ddc_golden_q15.h"// for the golden output data slot 245760 bytes
...
#define OUTPUT_OFFSET 0x40000// Offset for output data in BRAM
...
int main(void) {
    ...
    printf("LTE20 Mixer Test Application Started\n");
    printf("Input samples: %d\n", LTE20_UL_IF_SAMPLES);
    printf("Input bytes    : %d\n", LTE20_UL_IF_BYTES);
    printf("Output samples : %d\n", LTE20_UL_MIXER_DDC_OUT_SAMPLES);
    printf("Output bytes   : %d\n", LTE20_UL_MIXER_DDC_OUT_BYTES); 
    if (LTE20_UL_IF_SAMPLES !=61440) {
        printf("Error: Unexpected number of input samples. Expected 61440, got %d\n", LTE20_UL_IF_SAMPLES);
        return -1;
    }
    if (LTE20_UL_MIXER_DDC_OUT_SAMPLES != 15360) {
        printf("Error: Unexpected number of output samples. Expected 15360, got %d\n", LTE20_UL_MIXER_DDC_OUT_SAMPLES);
        return -1;
    }
    if(OUTPUT_OFFSET+LTE20_UL_MIXER_DDC_OUT_BYTES > BRAM_MAP_SIZE) {
        printf("Error: Output data exceeds BRAM map size. Output end address: 0x%X, BRAM map size: 0x%X\n", OUTPUT_OFFSET + LTE20_UL_MIXER_DDC_OUT_BYTES, BRAM_MAP_SIZE);
        return -1;
    }
    //open /dev/mem to access physical memory
    ...
    for (size_t i = 0; i < LTE20_UL_IF_SAMPLES; i++) {
        input_bram[i] = lte20_ul_if_q15[i];
    }
    //clear output area in BRAM
    for (size_t i = 0; i < LTE20_UL_MIXER_DDC_OUT_SAMPLES; i++) {
    ...
    DMA_REG(dma, DMA_S2MM_LENGTH_OFFSET) = LTE20_UL_MIXER_DDC_OUT_BYTES; // Set S2MM transfer length
    ...
    DMA_REG(dma, DMA_MM2S_LENGTH_OFFSET) = LTE20_UL_IF_BYTES; // Set MM2S transfer length
    ...
    for (size_t i = 0; i < LTE20_UL_MIXER_DDC_OUT_SAMPLES; i++) {
        ...
        uint32_t golden_sample = lte20_ul_mixer_ddc_golden_q15[i];

...
#include "lte20_duc_golden_q15.h"// for the input data slot 245760 bytes
#include "lte20_ddc_golden_q15.h"// for the golden output data slot 61440 bytes
...
int main(void) {
    ...
    for (size_t i = 0; i < LTE20_DUC_OUT_SAMPLES; i++) {
        input_bram[i] = lte20_duc_golden_q15[i];
    }
    //clear output area in BRAM
    for (size_t i = 0; i < LTE20_DDC_OUT_SAMPLES; i++) {
    ...
    DMA_REG(dma, DMA_S2MM_LENGTH_OFFSET) = LTE20_DDC_OUT_BYTES; // Set S2MM transfer length
    ...
    DMA_REG(dma, DMA_MM2S_LENGTH_OFFSET) = LTE20_DUC_OUT_BYTES; // Set MM2S transfer length
    ...
    for (size_t i = 0; i < LTE20_DDC_OUT_SAMPLES; i++) {
        ...
        uint32_t golden_sample = lte20_ddc_golden_q15[i];
